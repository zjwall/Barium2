...
from barium.lib.clients.Fiber_Switch_Client.fiber_switch_client2 import fiber_switch_client2
SIGNALID1 = 445567
SIGNALID2 = 445568
SIGNALID3 = 445569
SIGNALID4 = 445570
...
class software_laser_lock_client2(QWidget):
   
    def __init__(self, reactor, parent=None):
        super(software_laser_lock_client2, self).__init__()
    ...
    def connect(self):
        ...
        """
        print(multiplexer_config.ip)
        from labrad.wrappers import connectAsync
        self.password = os.environ['LABRADPASSWORD']
        self.wm_cxn2 = yield connectAsync('10.97.109.81', name =  socket.gethostname() + ' Single Channel Lock', password=self.password)
        self.wm2 = yield self.wm_cxn2.multiplexerserver
        ...
        # Get lasers to lock
        yield self.registry.cd(['Servers','software_laser_lock2'])
...
##        fiber_switch = fiber_switch_client2(self.reactor)
##        subLayout.addWidget(fiber_switch, 3,1,1,2)
    ...
    def set_signal_listeners(self):
        ...
        yield self.wm.addListener(listener = self.updateFrequency2, source = None, ID = SIGNALID1)
        yield self.wm2.signal__frequency_changed(SIGNALID2)
        yield self.wm2.addListener(listener = self.updateFrequency, source = None, ID = SIGNALID2)
    ...
    def updateFrequency(self, c, signal):
        for chan in self.lasers:
            if signal[0] == self.lasers[chan][1] and abs(signal[1] - self.lasers[chan][0]) < 12.5:# and self.lasers[chan][4] != 4:
    ...
    @inlineCallbacks
    def updateFrequency2(self, c, signal):
        for chan in self.lasers:
            if chan == "650nm":
                if signal[0] == self.lasers[chan][1] and abs(signal[1] - self.lasers[chan][0]) < 12.5:# and self.lasers[chan][4] != 4:
                    laser = self.channel_GUIs[chan]
...
if __name__ == "__main__":
    ...
    software_lock = software_laser_lock_client2(reactor)
    software_lock.show()
    reactor.run()
