...
class PMT_client(QWidget):
    ...
    def update_counts(self):
        ...
        """
        # get counts
        count_list = yield self.artiq.ttl_count_list(self.sample_time_us, self.num_samples)
    ...
    def toggle_polling(self, status):
        if status and (not self.refresher.running):
            # get timing values
            #we sample for 1 ms intervals so this is in counts/ms or kilocounts/sec and average over 10 samples not sure why we average
            poll_interval_s = .1
