...
##        self.auto_lock()
    ...
    def lock_0(self,ch_list):
        i=0
        self.lock_server.lock_channel(False,ch_list[i][0])
        self.lock_server.lock_channel(False,ch_list[i+1][0])

        self.changeChannel(self.channel_list[i][1])
        self.lock_server.lock_channel(True,ch_list[i][0])
        self.reactor.callLater(.1,self.lock_step_general,ch_list=self.ch_list,i=((i+1)%len(ch_list)))
    
    def lock_step_general(self,ch_list,i):
        print(i)
        self.lock_server.lock_channel(False,ch_list[(i+len(ch_list)-1)%len(ch_list)][0])
        self.changeChannel(self.channel_list[i][1])
        self.lock_server.lock_channel(True,ch_list[i][0])
        self.loop=self.reactor.callLater(self.ch_list[i][1]/1000.0,self.lock_step_general,ch_list= ch_list,i=((i+1)%len(ch_list)))
