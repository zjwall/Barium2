...
class pulse_sequence(object):
	...
	def __init__(self, parameter_dict, start = WithUnit(1.0, 'us')):
		...
		offset
		"""
		if not type(parameter_dict) == dict: raise Exception ("replacement_dict must be a TreeDict in sequence {0}".format(self.__class__.__name__))
		self.start = start
		self.end = start
		self._dds_pulses = []
		self._ttl_pulses = []
	...
	def addSequence(self, sequence, replacement_dict = {}, position = None):
		...
		if not type(replacement_dict) == dict: raise Exception ("replacement_dict must be a TreeDict")
