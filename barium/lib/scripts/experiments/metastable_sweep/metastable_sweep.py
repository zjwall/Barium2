...
class frequency_sweep(experiment):
    ...
    exp_parameters = [
                      ('1762Sweep133', 'Sequences_Per_Point'),
                      ('1762Sweep133', 'Start_Frequency'),
                      ('1762Sweep133', 'Stop_Frequency'),
                      ('1762Sweep133', 'Frequency_Step'),
                      ('1762Sweep133', 'dc_threshold'),
                      ]
    ...
    def initialize(self, cxn, context, ident):
        self.ident = ident
        self.cxn = labrad.connect(name = '1762 Sweep')
        self.cxnwlm = labrad.connect(multiplexer_config.ip, name = '1762 Sweep', password = 'lab')
        ...
        #### I believe we are going to need 
        ## self.cxn.hp8673m_server
        ...
        self.cycles = self.p.1762Sweep133.Sequences_Per_Point
        self.start_frequency = self.p.1762Sweep133.Start_Frequency
        self.stop_frequency = self.p.1762Sweep133.Stop_Frequency
        self.step_frequency = self.p.1762Sweep133.Frequency_Step
        self.state_detection = self.p.1762Sweep133.State_Detection
        self.dc_thresh = self.p.1762Sweep133.dc_threshold
        self.disc = self.pv.get_parameter('StateReadout','state_readout_threshold')
        ### change to 1762 pulse sequence
    ...
    def set_up_datavault(self):
        ...
        dataset = self.dv.new('1762Sweep_prob',[('freq', 'MHz')], [('Probability', 'Probability', 'num')], context = self.c_prob)
        # add dv params
        ...
        dataset1 = self.dv.new('1762Sweep_hist',[('freq', 'MHz')], [('Counts', 'Counts', 'num')], context = self.c_hist)
        # add dv params
        for parameter in self.p:
        ...
        dataset2 = self.dv.new('1762Sweep_dc_hist',[('run', 'arb u')],\
