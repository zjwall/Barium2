...
### BEGIN NODE INFO
[info]
name = piezo_controller2
version = 1.0
description =
instancename = piezo_controller2
...
class piezo_server(DeviceServer):
    ...
    def loadConfigInfo(self):
        ...
        reg = self.reg
        yield reg.cd(['', 'Servers', 'piezo_controller2', 'Links'], True)
        ...
        yield reg.cd(['', 'Servers', 'piezo_controller2', 'parameters'], True)
        dirs, keys = yield reg.dir()
        p = reg.packet()
        for k in keys:
            p.get(k, key=k)
